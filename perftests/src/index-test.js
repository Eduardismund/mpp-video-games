...
function generateData(){
  ...
  if (videoGameId) {
    ...
    while (reviews > 0) {
      createVideoGameReview({videoGameId})
      reviews--
    }
  }
}
...
function createVideoGame() {
  ...
  const res = http.post(`${urlBase}/api/video-games`,
    JSON.stringify(generateSingularVideoGame({genres})), {
      headers: {'Content-Type': 'application/json'}, tags: {type: "createVideoGame"}
    })
  if (res.status === 201) {
 ...
 */
function createVideoGameReview({videoGameId}) {
  const res = http.post(`${urlBase}/api/video-games/${videoGameId}/reviews`,
    JSON.stringify(generateSingularVideoGameReview()), {
      headers: {'Content-Type': 'application/json'}, tags: {type: "createVideoGameReview"}
    })
